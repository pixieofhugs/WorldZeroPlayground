...
**An on-scale value can never be ornament.** The carve-out exists because *rounding* an off-scale value reflows the composition. A value already sitting on a rung is not rounded at all, so that justification is absent and the claim is empty. Before hatching, check the number: `padding: 4` is `--space-xs` and simply takes the token, however ornamental its surroundings. A legitimate claim is **off-scale** *and* **in register with raw ornament geometry** — the canonical case is a `paddingLeft: 46` clearing a *drawn* margin rule at `left: 32`, where rounding to a rung runs body text into the line. That is correctness, not taste.
