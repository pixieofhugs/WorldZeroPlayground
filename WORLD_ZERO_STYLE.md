...
Read this next to the Albescent note above: those are the repo's two partly-registered factions, partial on **different axes**. Albescent has neither theme nor skin, on purpose, because it is hiding. WOW now has both, and eight manifest surfaces claim it (praxis card, its mobile twin, the score stamp, the vote widget, the edit-praxis composer on both form factors, and — since #897 — the sigil and the avatar), so the rest still render `Default*`.
