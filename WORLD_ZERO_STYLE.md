...
**Where a role has one ink, two roles collapse onto it, and that is a decision to write down rather than a thing to notice later.** `-card-notice` is the card family's whole attention role, so danger and warning print the same colour on this surface: the flagged and failed badges (mutually exclusive) and the hide/fail controls (side by side) lose their red/amber severity cue. Nothing is owed to WCAG 1.4.1 — each mark carries its own word — and minting a `-card-alarm` sibling for eight keys in two themes to keep a distinction that was failing AA on every sheet is the "no new token family if an existing per-sheet ink already covers the pairing" rung. The upgrade path is named at `sheetInk`'s declaration; **a deliberate flattening gets a `ponytail:` comment at the seam, not a silence.**
