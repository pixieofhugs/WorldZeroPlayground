...
**What the peers share with it is polarity, not one palette — and this is the repo's first skin with no night tuning at all.** Being exact matters here, because the loose version of the sentence ("the way Singularity and S.N.I.D.E. already are") is simply false, and it would license the next faction to skip a measurement on the strength of a pattern that does not exist. S.N.I.D.E. and Singularity are the other two skins that are dark in *both* cascades (§6), and both are still **tuned per theme**: Singularity restates all seventeen `--faction-singularity-term-*` tokens under `[data-theme="dark"]` with different values (`-bg` #07130c against #050f08, `-ink` #22c55e against #4ade80, `-blue` #4c7fef against #60a5fa), and S.N.I.D.E. is a real precedent for its `-ink`/`-paper` pair while its `-wall-*` family flips outright. Ephemerists is the first register whose dark cascade declares nothing — a claim about *this* palette, drawn for the night and measured on both app grounds, not a shape to copy on sight. What still flips for the faction is its spine hue's fill, wash and border, which are not the skin.
