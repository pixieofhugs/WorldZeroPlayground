...
Read this next to the Albescent note above: those are the repo's two partly-registered factions, partial on **different axes**. Albescent has neither theme nor skin, on purpose, because it is hiding. WOW now has both, and eleven manifest surfaces claim it (praxis card, its mobile twin, the score stamp, the vote widget, the edit-praxis composer on both form factors, the sigil and the avatar since #897, and — since #899 — the task card, the comment and the feed frame), so the rest still render `Default*`.
