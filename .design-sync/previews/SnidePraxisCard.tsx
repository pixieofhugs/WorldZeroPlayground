// SnidePraxisCard preview cells — the desktop praxis card: a submitted proof as it
// appears in a feed or a task's gallery. Chrome is this faction's own, so the
// content is bound to 'snide' and the variants sweep the states a reader meets.
import { SnidePraxisCard } from 'worldzero-frontend'
...
export function Submitted() {
  const praxis = makePraxisCard({ task_faction_slug: 'snide' })
  return (
    <div style={wrap}>
      <SnidePraxisCard praxis={praxis} adminProps={adminPropsFor(praxis)} />
    </div>
  )
}
...
export function Unscored() {
  ...
  return (
    <div style={wrap}>
      <SnidePraxisCard praxis={praxis} adminProps={adminPropsFor(praxis)} showCrown={false} />
    </div>
  )
}
