// WowScoreStamp preview cells — the score mark a praxis card wears. Small and
// signature, so each cell is one stamp and the sweep is the states that change
// what it draws: crowned, plain, and unscored.
import { WowScoreStamp } from 'worldzero-frontend'
...
export function Crowned() {
  return (
    <div style={wrap}>
      <WowScoreStamp praxis={makePraxisCard({ task_faction_slug: 'wow' })} />
    </div>
  )
}
...
export function Scored() {
  return (
    <div style={wrap}>
      <WowScoreStamp
        praxis={makePraxisCard({ task_faction_slug: 'wow', score: 36, voter_count: 4, is_top_for_task: false, points_from_votes: 6 })}
      />
    </div>
  )
}
...
export function CrownSuppressed() {
  return (
    <div style={wrap}>
      <WowScoreStamp praxis={makePraxisCard({ task_faction_slug: 'wow' })} showCrown={false} />
    </div>
  )
}
...
export function Unscored() {
  return (
    <div style={wrap}>
      <WowScoreStamp
        praxis={makePraxisCard({ task_faction_slug: 'wow', score: 0, points_from_votes: 0, voter_count: 0, is_top_for_task: false })}
      />
    </div>
  )
}
