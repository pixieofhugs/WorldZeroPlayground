// EphemeristsPraxisCard preview cells — the desktop praxis card: a submitted proof as it
// appears in a feed or a task's gallery. Chrome is this faction's own, so the
// content is bound to 'ephemerists' and the variants sweep the states a reader meets.
import { EphemeristsPraxisCard } from 'worldzero-frontend'
...
export function Submitted() {
  const praxis = makePraxisCard({ task_faction_slug: 'ephemerists' })
  return (
    <div style={wrap}>
      <EphemeristsPraxisCard praxis={praxis} adminProps={adminPropsFor(praxis)} />
    </div>
  )
}
...
export function Unscored() {
  ...
  return (
    <div style={wrap}>
      <EphemeristsPraxisCard praxis={praxis} adminProps={adminPropsFor(praxis)} showCrown={false} />
    </div>
  )
}
