// AlbescentPraxisCard preview cells — the desktop praxis card: a submitted proof as it
// appears in a feed or a task's gallery. Chrome is this faction's own, so the
// content is bound to 'albescent' and the variants sweep the states a reader meets.
import { AlbescentPraxisCard } from 'worldzero-frontend'
...
export function Submitted() {
  const praxis = makePraxisCard({ task_faction_slug: 'albescent' })
  return (
    <div style={wrap}>
      <AlbescentPraxisCard praxis={praxis} adminProps={adminPropsFor(praxis)} />
    </div>
  )
}
...
export function Unscored() {
  ...
  return (
    <div style={wrap}>
      <AlbescentPraxisCard praxis={praxis} adminProps={adminPropsFor(praxis)} showCrown={false} />
    </div>
  )
}
