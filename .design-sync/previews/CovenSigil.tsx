// CovenSigil preview cells — the faction's mark on its own. Tiny by nature, so
// each cell is a size run: the sizes it actually appears at across the site.
import { CovenSigil } from 'worldzero-frontend'
...
export function Tinted() {
  return (
    <div style={row}>
      <CovenSigil size={64} color="var(--color-text-primary)" />
      <CovenSigil size={64} color="var(--color-accent-primary)" />
    </div>
  )
}
