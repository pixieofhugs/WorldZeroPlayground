...
const TOKENS = {
  light: {
    ...
    ochre:'#A6432B', nile:'#2E6E6A',
    plate:'#EADCB6', plate2:'#F2E7CB', band:'#1E2233', bandInk:'#EFE4C4', disc:'#F6EED6',
    rule:'rgba(42,36,24,0.10)', line:'rgba(42,36,24,0.24)',
...
function EphemeristsTaskDetail(props) {
  ...
  // fluted rule — the cavetto band, reused as a divider
  const fluted = (extra) => el('div', { 'aria-hidden':true, style:{ display:'flex', alignItems:'flex-start', gap:3, height:7, overflow:'hidden', ...(extra||{}) } },
    Array.from({ length: 48 }).map((_, i) => el('span', { key:i, style:{ flex:1, height: i % 2 ? 7 : 4, background:t.brass, opacity: i % 2 ? .5 : .28 } })));
  ...
  const wing = (dir) => el('g', { transform: dir === 'l' ? 'scale(-1,1)' : 'none' },
    [0,1,2,3,4].map((i) => el('rect', { key:i, x:13 + i*11.4, y:-6 + i*1.5, width:9.6, height:8.4 - i*1.4, rx:1.4, fill:t.gold, opacity:.9 - i*0.13 })),
    [0,1,2,3].map((i) => el('rect', { key:'b'+i, x:15 + i*11.4, y:4.2 + i*1.6, width:8, height:3.4 - i*0.5, rx:1, fill:t.brassL, opacity:.62 - i*0.11 })));
  ...
  const oct = (inset, stroke, width, fill) => el('path', {
    d:'M30 4 L70 4 L96 30 L96 70 L70 96 L30 96 L4 70 L4 30 Z',
    fill: fill || 'none', stroke, strokeWidth:width,
  ...
  const medallion = el('div', { style:{ flex:'0 0 auto', position:'relative', width:medSize, height:medSize, display:'flex', alignItems:'center', justifyContent:'center' } },
    el('svg', { width:medSize, height:medSize, viewBox:'0 0 100 100', 'aria-hidden':true, style:{ position:'absolute', inset:0 } },
      oct(0, t.brass, 1.6, t.disc),
      oct(6, t.brassL, 0.7),
      el('circle', { cx:50, cy:50, r:34, fill:'none', stroke:t.brassL, strokeWidth:0.7, opacity:.55 }),
      el('path', { d:'M16 70 H84', stroke:t.brassL, strokeWidth:.7, opacity:.5 })),
    el('div', { style:{ position:'relative', display:'flex', flexDirection:'column', alignItems:'center', lineHeight:.82 } },
      el('span', { style:{ fontFamily:DECO, fontSize: desktop ? 40 : 34, color:t.ink } }, yourPoints),
      el('span', { style:{ ...sc, fontSize:7, letterSpacing:'.2em', color:t.muted, marginTop:6 } }, 'points')));
  ...
  const crownedPanel = (panel) => el('div', { style:{ position:'relative', paddingTop: desktop ? 96 : 74 } },
    el('svg', { width: desktop ? 400 : 290, height: desktop ? 91 : 66, viewBox:'-88 -20 176 40', 'aria-hidden':true,
      style:{ position:'absolute', top:0, left:'50%', transform:'translateX(-50%)', zIndex:2, pointerEvents:'none',
        filter: theme === 'dark' ? 'drop-shadow(0 3px 7px rgba(0,0,0,.6))' : 'drop-shadow(0 2px 0 #1E2233) drop-shadow(0 6px 14px rgba(30,34,51,.45))' } },
      wing('r'), wing('l'),
      el('circle', { r:11, fill:t.disc, stroke:t.brass, strokeWidth:1.6 }),
      el('circle', { r:5.5, fill:t.gold, opacity:.85 })),
    el('div', { style:{ position:'relative', zIndex:1 } }, panel));

  const actionPanel = el('div', { style:{ ...plate, outline:`1px solid ${t.brassL}`, outlineOffset:3, boxShadow:t.shadow, padding:7,
  ...
  // ── brief, on a ruled journal leaf with a red margin rule ─────────────
  const brief = el('section', { style:{ marginBottom: desktop ? 34 : 26 } },
    sectionHead('Task description', 'in full'),
    el('div', { style:{ ...plate, position:'relative', padding: desktop ? '22px 26px 20px 40px' : '18px 18px 16px 30px',
      backgroundImage:`repeating-linear-gradient(180deg, transparent 0 27px, ${t.rule} 27px 28px)`, backgroundPosition:'0 6px' } },
      el('span', { 'aria-hidden':true, style:{ position:'absolute', top:0, bottom:0, left: desktop ? 26 : 19, width:1, background:t.ochre, opacity:.5 } }),
      d.brief.map((p, i) => el('p', { key:i, style:{ ...body(desktop ? 15.5 : 14), color:t.ink, margin: i === d.brief.length - 1 ? 0 : '0 0 14px', textWrap:'pretty' } }, p))));
