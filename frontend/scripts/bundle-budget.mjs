 ...
 *
 * JS ledger (#2843): warn stays at 134,000, against a measured 155,659 ->
 * 152,046. CSS was measured too and was not touched: 22,378, `ok`, 322 bytes
 * under its line.
 *
 * THE FIRST JS ENTRY SINCE #1400, AND THE FIRST WHERE A REAL WIN CHANGES NO
 * LINE. Every CSS entry above ends by deciding how far WARN follows the cut
 * down. This one cannot ask that question: 152,046 is still 18,046 bytes ABOVE
 * warn, so there is no rung to ratchet into — the line only moves when the
 * number goes under it, and a 3,613-byte cut against a 21,659-byte overage does
 * not get there. `main` keeps printing the WARN block, which is exactly the
 * standing ruling on #2843: bytes come off, the line does not move, and a
 * WARN nothing is blocked by is accepted while the hunt continues.
 *
 * WHERE THE 3,613 CAME FROM. 2,979 of it is two event popups leaving the eager
 * path. `LevelUpWatcher` and `InvitationWatcher` are mounted in `Layout`, so
 * they run on every page, and both are a localStorage diff that returns `null`
 * on effectively every load — a player crosses a level or earns an invitation a
 * handful of times in an era. Each reached its popup by a STATIC import, so
 * every visitor of every page paid for a Field Stamp and a recruitment
 * prospectus, plus what the letter drags behind it. The diff stays blocking
 * because it is what decides whether anything fires; the popup is now behind
 * `lazy()`. `eagerPathImports.test.ts` holds it, for that file's own reason:
 * this check CAN see the win but cannot HOLD it, because JS is over WARN and
 * WARN exits 0, so the number would drift back up printing this same block with
 * nothing going red.
 *
 * THE OTHER 634 BYTES ARE NOT THE POINT OF THE OTHER HALF, and should not be
 * read as one. `vite.config.ts` declared no `manualChunks`, so the ~41 KB
 * `react-dom-*.js` chunk this file priced for months was Rollup's AUTOMATIC
 * split; React 19 (#2920) changed the module graph's shape and it dissolved
 * into the entry chunk. Restoring it by intent recovers only the three collapsed
 * shim wrappers in bytes. What it actually repairs is invisible here BY
 * CONSTRUCTION: this check measures a cold first load, where moving code between
 * two blocking chunks is neutral, and the regression was on the second visit —
 * a content-hashed vendor chunk survives an app-code deploy and the entry chunk
 * does not, so a returning visitor re-fetched 58.8 KB gzipped of unchanged React
 * runtime every time `main` auto-deployed. `reactVendorChunk.test.ts` is that
 * one's ratchet, and it is a source read rather than a `dist/` read because CI
 * runs vitest before the build.
 *
 * THE CANDIDATES EXAMINED AND NOT AVAILABLE, so the next pass does not re-derive
 * them. Measured by attributing the entry chunk's generated bytes to source
 * modules through its sourcemap, which is the inventory this issue was missing.
 *
 *  - `FactionSigil`'s INEFFECTIVE_DYNAMIC_IMPORT, which the thread named as the
 *    concrete lead: NOT a defeated win. Four faction modules `import()` it while
 *    `Sidebar` imports it statically, and `Sidebar` is right — it draws the
 *    sigil on every page, as `eagerPathImports.test.ts` already says out loud.
 *    The module is 475 generated chars. The four dynamic imports are redundant,
 *    not thwarted; the warning is noise here, not a lead.
 *  - `react-router`, 37,867 chars and the largest single item left: the router
 *    must resolve before anything paints.
 *  - `Sidebar.tsx`, 12,849 chars, and `normalizeFeedItem` (5,849) and
 *    `api/praxis` (2,473) which are on the path only because it reaches them:
 *    the rail's three panels are drawn on first paint on every route. Deferring
 *    the feed panel is a real ~8 KB, but it trades bytes for the rail popping in
 *    after load — a visible layout decision, not a byte cut, and it needs the
 *    visual QA this pass could not do.
 *  - The nine `src/factions/*.ts` registries, 25,138 chars together and the
 *    biggest app-code cluster remaining: the honest next candidate, and
 *    deliberately not taken here. Seven parallel lanes were appending
 *    registration lines to those exact nine files while this was measured, and
 *    the shell resolves the registry synchronously, so making it lazy is an
 *    architecture change rather than a boundary move.
 */
const BUDGETS = {
