 ...
 * asked for the page modules at all while the session was still resolving.
 *
 ...
 *
 * It also does NOT prove "no second round trip": `React.lazy` suspends on its
 * first render even when the module is already in the ES module cache (see the
 * long comment in `factions/lazyArchetype.tsx`), so warmed and cold look
 * identical in markup. `renderResolved` below pays that one microtask
 * deliberately; in the browser it is a microtask too, not a fetch.
 *
 * The second half is the guard that matters more: reordering work around an
 * auth gate is how a signed-out viewer ends up seeing a signed-in surface for a
 * frame. `RootLanding` must render FieldDesk only for a resolved signed-in
 * session, Home only for a resolved signed-out one, and neither while loading.
 */
...
describe('the root route starts its chunk before the auth answer (#1380)', () => {
  it('imports both landing pages while the session is still loading', async () => {
    // A fresh module registry per test file, and this is the first render in
    // the file: nothing else can have imported these two.
    expect({ ...imported }).toEqual({ home: false, fieldDesk: false })

    renderRoot({ user: null, loading: true })
    await nextTick()

    expect({ ...imported }).toEqual({ home: true, fieldDesk: true })
  })
})
...
describe('the root route shows no signed-in surface it should not', () => {
  ...
  it('renders Home for a resolved signed-out viewer', async () => {
