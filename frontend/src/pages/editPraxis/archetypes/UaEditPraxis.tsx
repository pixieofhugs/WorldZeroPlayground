...
const SHEET = "var(--edit-praxis-paper, var(--faction-ua-card-bg))"; /* the sun-bleached sheet */
...
const INK = "var(--edit-praxis-ink, var(--faction-ua-card-text))";
...
const ACCENT = "var(--edit-praxis-accent, var(--faction-ua-card-accent))"; /* the design's accentDeep */
const RULE = "var(--faction-ua-rule)"; /* the neutral hairline */
const HAIR = "var(--faction-ua-hair)"; /* the faintest divider, below -rule */
const FILL = "var(--edit-praxis-fill, var(--faction-ua))";
const ON_FILL = "var(--edit-praxis-on-fill, var(--faction-ua-on-fill))";
...
export default function UaEditPraxis({ state }: Props) {
  ...
  const dress: ComposerDress = {
    ...
    pageStyle: { ...factionRoleVars("ua", "edit-praxis"), fontFamily: UA_TEXT, color: INK },
    sheetStyle,
    ground: groundLayer,
    rule: () => rule,
    mark: statusMark,
